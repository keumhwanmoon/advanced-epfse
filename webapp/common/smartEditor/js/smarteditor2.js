...
//		jindo.$Element(this.toolbarArea).addClass("off");
	...
	$ON_EVENT_TOOLBAR_MOUSEDOWN : function(weEvent){
		...
		// Check if the button pressed is in active status and has a visible layer i.e. the button had been clicked and its layer is open already. (buttons like fonts styles-bold, underline-got no sub layer -> childNodes.length<=2)
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
	...
	_getFontFamilyFromLI : function(elLi){
		...
		// <li><button type="button"><span>돋음</span>(</span><em style="fonts-family:'돋음',Dotum,'굴림',Gulim,Helvetica,Sans-serif;">돋음</em><span>)</span></span></button></li>
	 ...
	 * @param fontId {String} value of fonts-family in style
	 * @param fontName {String} name of fonts list in editor
	 * @param defaultSize 
	 * @param fontURL 
	 * @param fontCSSURL 
	 * @param fontType fontType == null, custom fonts (sent from the server)
	 *                 fontType == 1, default fonts
	 *                 fontType == 2, tempSavedFont
	 * @param sSampleText {String} sample text of fonts list in editor
	 * @param bCheck {Boolean} 
	 */
	addFont : function (fontId, fontName, defaultSize, fontURL, fontCSSURL, fontType, sSampleText, bCheck) {
		// custom fonts feature only available in IE
...
/*
		// do not add again, if the fonts is already in the list
...
*/		
		...
		// [원인]
		// 		- IE9의 웹폰트 로드/언로드 시점
		// 			웹폰트 로드 시점: StyleSheet 의 @fonts-face 구문이 해석된 이후, DOM Tree 상에서 해당 웹폰트가 최초로 사용된 시점
		// 			웹폰트 언로드 시점: StyleSheet 의 @fonts-face 구문이 해석된 이후, DOM Tree 상에서 해당 웬폰트가 더이상 사용되지 않는 시점
		// 		- 메뉴 리스트에 적용되는 스타일은 @fonts-face 이전에 처리되는 것이어서 언로드에 영향을 미치지 않음
		//
		...
		// [해결] 
		//		언로드가 발생하지 않도록 메뉴 리스트에 스타일을 적용하는 것을 @fonts-face 이후로 하도록 처리하여 DOM Tree 상에 항상 적용될 수 있도록 함
		//
	...
	// Add the fonts AND load it right away
	...
	// Add the fonts AND load it right away AND THEN set it as the default fonts
...
function fontProperty(fontId, fontName, defaultSize, fontURL, fontCSSURL){
	...
	if(this.fontCSSURL != ""){
		...
		// load the css that loads the custom fonts
		this.loadCSS = function(doc){
			// if the fonts is loaded already, return
			if(this.isLoaded){
				return;
			}
...
*/
			// FF3 will actually display %uFEFF when it is followed by a number AND certain fonts-family is used(like Gulim), so remove the character for FF3
...
//			jindo.$Element(aCellInfo[0]).css("height", aCellInfo[2]);
	...
	_createCellResizeGrip : function(){
		...
		try{
			this.elTmp.innerHTML = '<div style="position:absolute; overflow:hidden; z-index: 99; "><div onmousedown="return false" style="background-color:#000000;filter:alpha(opacity=0);opacity:0.0;-moz-opacity:0.0;-khtml-opacity:0.0;cursor: col-resize; left: 0px; top: 0px; width: 100%; height: 100%;fonts-size:1px;z-index: 999; "></div></div>';
...
(function() {
	...
	IsInstalledFont = function(sFont) {
		...
		var sStyle = 'position:absolute !important; fonts-size:200px !important; left:-9999px !important; top:-9999px !important;';
		oDummy.innerHTML = 'mmmmiiiii'+unescape('%uD55C%uAE00');
		oDummy.style.cssText = sStyle + 'fonts-family:"' + sDefFont + '" !important';
		...
		oDummy.style.cssText = sStyle + 'fonts-family:"' + sFont.replace(rx, '","') + '", "' + sDefFont + '" !important';
