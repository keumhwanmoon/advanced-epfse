...
function fnInqr()
{
    gfnReqGrid( // 그리드 요청
        ...
        g_strGridId
        // URI
      , "/system/getSysVarList.do" // 목록 가져오기
...
function fnSave()
{
    gfnReq( // 요청
        // URL
        "/system/saveSysVarList.do" // 목록 저장
        // 데이터
      ...
      , function(objData)
        {
            ...
            if ( Base.OK != RSLT_VALUE )
            {
                ...
                var strSffx = gfnGetJsonValue(objData, [ Base.RSLT_INFO, "varId" ]);
                if ( !gfnIsEmpty(strSffx) ) strSffx = '${requestScope["ITEM.varId"]}<%-- 변수ID --%> = ' + strSffx;

                if ( Base.DATA_DUP == RSLT_VALUE )
                    gfnSetGridCellFocus(g_strGridId, DATA_ROW, "varId", function() // 그리드 셀 포커스를 설정한다.
                ...
                if ( !gfnIsBaseError(RSLT_VALUE) )
                    gfnSetGridCellFocus(g_strGridId, DATA_ROW, null, function() // 그리드 셀 포커스를 설정한다.
                        { gfnDispMsg(Base.msgRsltErrorBase.msgRsltError, strSffx); }); // 요청결과 오류가 발생했습니다. 관리자에게 문의하십시오.
            } else
            {
...
function fnDelt()
{
    gfnReq( // 요청
        // URL
        "/system/deltSysVarList.do" // 목록 삭제
        // 데이터
...
{
    // FIELD 정보를 설정한다.
    g_arrFieldName = "rowId|varId|varName|intrrName".split(Base.DELI1);
    ...
    gfnInitGrid(g_strGridId, g_arrFieldName, null, Base.GRID,
        {
          ...
          , "columns": [
              ...
              , { "header": { "text": '${requestScope["ITEM.varId"    ]}<%-- 변수ID --%>' }, "width": 100, "fieldName": g_arrFieldName[1], "editor": { "maxLength": 20 } }
              , { "header": { "text": '${requestScope["ITEM.varName"  ]}<%-- 변수명 --%>' }, "width": 200, "fieldName": g_arrFieldName[2], "editor": { "maxLength": 100 } }
              , { "header": { "text": '${requestScope["ITEM.intrrName"]}<%-- 내부명 --%>' }, "width": 200, "fieldName": g_arrFieldName[3], "editor": { "maxLength": 100 } }
            ]
        // 이벤트관련
        ...
        if ( !gfnCallGridChcRowFunc(g_strGridId, function(objGv, objRow, numItemIndex) // 그리드 선택 열별 셀 값을 검증한다.
            {
                return gfnVerifGridRow(objGv, objRow, numItemIndex, [ // 그리드 행 검증
                    {
                        "text": '${requestScope["ITEM.varId"]}<%-- 변수ID --%>', "fieldName": "varId"
                  ...
                  , {
                        "text": '${requestScope["ITEM.varName"]}<%-- 변수명 --%>', "fieldName": "varName"
                      , "msgInputYn": '${requestScope["ITEM.msgInputItem"]}<%-- 해당 항목을 입력하십시오. --%>'
