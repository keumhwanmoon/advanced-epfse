...
var g_objTable = // 테이블
    {
      ...
      , "alignLeft":
            [ false, true, false, false, false ] // 좌측정렬여부
        // 이벤트관련
...
function fnInqr()
{
    gfnReq( // 요청
        // URL
        "/system/getFilermList.do" // 목록 가져오기
        // 데이터
...
{
    // 페이지 이동
    gfnMovePage("/system/filermDtl.do?rowId=" + encodeURIComponent(strRowId)); // 상세
}
...
{
    // 페이지 이동
    gfnMovePage("/system/filermEdit.do"); // 편집
}
...
{
    var strFilermTitle = strData;

    if ( objRow["newYn"] == Base.YES )
    {
        strFilermTitle = '<img src="${pageContext.request.contextPath}/common/images/common/ico_new.gif" /> ' + strFilermTitle;
    }

    return strFilermTitle;
}
...
<body>
    ...
    <div id="divList" class="tableWrap">
        <table class="boardList01" style="margin-right: 20px">
