...
var g_objTable = // 테이블
    {
      ...
      , "colId": // 바인딩 레코드 VO변수명
            [ "bltnNo", "titleName", "rgstUserName", "rgstDtm", "answrExist", "rowId" ]
      , "colStyle": // 열스타일
            [ null, "text-align: left", null, null, null, "display: none" ]
      , "colFormt": // 열포맷
            [ null, fnSetTitle, null, gfnFormtTableDtm, fnSetState, null ]
      , "alignLeft":
            [ false, true, false, false, false, false ] // 좌측정렬여부
        // 이벤트관련
...
{
    // 페이지 이동
    gfnMovePage("/system/qnaDtl.do?rowId=" + encodeURIComponent(strRowId)); // 상세
}
...
{
    // 페이지 이동
    gfnMovePage("/system/qnaEdit.do"); // 편집
}
...
{
    var strQnaTitle = strData;

    if ( objRow["newYn"] == Base.YES )
    {
        strQnaTitle = '<img src="${pageContext.request.contextPath}/common/images/common/ico_new.gif" /> ' + strQnaTitle;
    }
...
// 상태여부 설정
//-------------------------------------------------------------------------------
function fnSetState(strData, objRow)
{
    var strState = Base.EMPTYSTR;
    var strAnswrExist = strData;

    if( !gfnIsEmpty(strAnswrExist) )
    {
        strState = '<span class="reply status02">완료</span>';
    }
    else
    {
        strState = '<span class="reply status01">대기</span>';
    }
...
<body>
    ...
    <div class="buttonBox right" style="margin-top: 15px">
