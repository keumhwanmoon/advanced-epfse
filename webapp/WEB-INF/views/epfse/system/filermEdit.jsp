...
function fnSave()
{
    ...
    (
        // URL
        "/system/saveFilermInfo.do" // 정보 저장
        // 데이터
...
function fnCncl()
{
    ...
    {
        // 페이지 이동
        gfnMovePage("/system/filermList.do"); // 목록
    } else
    {
        gfnSetScrParam("inqrCountUpdtYn", Base.NO); // 조회수

        // 페이지 이동
        gfnMovePage("/system/filermDtl.do?rowId=" + encodeURIComponent(ROW_ID)); // 상세
    }
}
...
function fnCtrlScr(strClsfy)
{
    if ( "READY" == strClsfy ) // DOM준비
    {
        ...
        if ( 0 >= ROW_ID.length )
            // EDIT 를 초기화한다.
            gfnBindEditValue("txaFilermCtts"); // EDIT 값 바인딩
    } else
...
function fnBindEdit(objRsltInfo)
{
    ...
    if ( 0 < ROW_ID.length )
    {
        ...
        gfnBindEditValue("txaFilermCtts", txaFilermCtts.value); // EDIT 값 바인딩
    }
}
            ...
            <tbody>
                <tr>
                    <td colspan="2" class="content">
                        <textarea id="txaFilermCtts" name="ctts" cols="100" rows="10" class="inputText"></textarea>
