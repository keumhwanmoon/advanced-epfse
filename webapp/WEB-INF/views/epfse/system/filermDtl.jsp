...
function fnInqrAttch()
{
    ...
    (
        // URL
        "/data/getFilermAttchList.do" // 목록 조회
        // 데이터
      , gfnGetFormData("divInqrCond")
        // 성공콜백함수
      , function(objData)
...
function fnDelt()
{
    ...
    (
        // URL
        "/system/deltFilermInfo.do" // 정보 삭제
        // 데이터
      , gfnGetFormData("divInqrCond")
        // 성공콜백함수
      , function(objData)
...
function fnUpdt()
{
    ...
    // 페이지 이동
    gfnMovePage("/system/filermEdit.do?&rowId=" + encodeURIComponent(ROW_ID)); // 편집
}
...
{
    // 페이지 이동
    gfnMovePage("/system/filermList.do");
}
...
{
    // 페이지 이동
    gfnMovePage("/system/filermDtl.do?rowId=" + encodeURIComponent(strPreRowId)); // 상세
}
...
{
    if ( "DELT" == strClsfy || "UPDT" == strClsfy ) // 삭제 // 수정
    {
                ...
                <tr>
                    <td colspan="3" class="content">
                        ...
                        <div class="AttchContnr">
                            ...
                            <div class="AttchBtn">
                                <div class="Link">
                                    <a style="display: inline" href="#" onclick='javascript: gfnClickAttchDownload();'>${requestScope["ITEM.download"]}<%-- 다운로드 --%></a>
                                </div>
                                <div class="AttchCount">
                                    <span>${requestScope["ITEM.attchMaxCount"]}<%-- 첨부 횟수 제한 --%>:
                                    </span><span class="CrrntCount">0</span> / <span class="MaxCount">3</span>
                                </div>
                                <div class="AttchSize">
                                    <span>${requestScope["ITEM.fileMaxSize"]}<%-- 파일 크기 제한 --%>:
                                    </span><span class="MaxSize">${requestScope["SYS_VAR.attchFileMaxSize"]}</span>
                                    <span>MB</span>
                                </div>
                            </div>
