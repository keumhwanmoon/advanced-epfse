...
--------------------------------------------------------------------------------
*/
package com.ecosian.epfse.member.auth.ctrl;
...
{
    @Resource(name = "userAuthService") // 해당 beanName 과 일치하는 bean 을 가져온다.
    private UserAuthService svc;

    // 페이지경로 가져오기 // 사용자권한 그리드 화면 요청시 사용한다.
    @RequestMapping(value="/member/userAuthGrid.do")
    ...
    // 목록 가져오기 // 사용자권한 그리드 화면 조회 클릭시 사용한다.
    @RequestMapping(method=RequestMethod.POST, value="/member/getUserAuthList.do")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody Channel getList(HttpServletRequest req, @ModelAttribute ComParamVO voComParam,
            @ModelAttribute UserAuthVO vo)
    {
        return svc.getList(vo);
    }

    // 목록 등록 // 사용자권한 목록 화면 추가 클릭시 사용한다.
    @RequestMapping(method=RequestMethod.POST, value="/member/rgstUserAuthList.do")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody Channel rgstList(HttpServletRequest req, @ModelAttribute ComParamVO voComParam,
            @ModelAttribute UserAuthVO vo)
    {
        vo.setRgstUserId(vo.getComParamLoginUserId());
        return svc.rgstList(vo);
    }

    // 목록 삭제 // 사용자권한 그리드 화면 삭제 클릭시 사용한다.
    @RequestMapping(method=RequestMethod.POST, value="/member/deltUserAuthList.do")
    @ResponseStatus(HttpStatus.OK)
