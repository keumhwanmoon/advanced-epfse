...
public class NotiAttchController
{
    @Resource(name = "notiAttchService")
    private NotiAttchService svc;

    // 목록 가져오기 // 게시물 상세 화면 로드시 사용한다. // 게시물 편집 화면 로드시 사용한다.
    @RequestMapping(method=RequestMethod.POST, value="/data/getNotiAttchList.do")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody Channel getList(HttpServletRequest req,@ModelAttribute ComParamVO voComParam,
            @ModelAttribute NotiAttchVO vo)
    {
